...
// Firebase Data Paths
const paymentsRef = database.ref("tracollector/Gfunds");
const merchantsRef = database.ref("tracollector/merchants");
const usersRef = database.ref("tracollector/users");
const paymentTypesRef = database.ref("tracollectorGfunds");
...
const newPaymentRef = firebase.database().ref('Gfunds').push();
const newPaymentKey = newPaymentRef.key;

firebase.database().ref('Gfunds/' + newPaymentKey).set(newPayment)
    .then(() => {

 
        Swal.fire({
        title: "Success!",
        text: "New transfer added successfully!",
        icon: "success",
