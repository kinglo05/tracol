...
function buildRow(paymentId, p) {
  ...
  cb.addEventListener("change", async (ev) => {
    ...
    try {
      ...
      await db.ref(`Gfunds/${paymentId}`).update({ status: newStatus });
...
async function computeTodayNewTotal() {
  try {
    ...
    const q = db.ref("Gfunds").orderByChild("date").startAt(todayISO).endAt(todayISO);
...
if (btnSaveEdit) {
  btnSaveEdit.addEventListener("click", async () => {
    try {
      ...
      await db.ref(`Gfunds/${id}`).update(payload);
