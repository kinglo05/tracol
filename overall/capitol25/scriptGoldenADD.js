...
function saveCollection1() {
  ...
  const addressIn = document.getElementById("collectionAddress");
  ...
  const newPaymentRef = firebase.database().ref('capitol2025/transactions').push();
  const newPaymentKey = newPaymentRef.key;

  firebase.database().ref('capitol2025/transactions/' + newPaymentKey).set(newTrans)
    .then(() => {
...
function saveExpenses() {
  ...
  const newPaymentRef = firebase.database().ref('capitol2025/goldenExpenses').push();
  const newPaymentKey = newPaymentRef.key;

  firebase.database().ref('capitol2025/goldenExpenses/' + newPaymentKey).set(newPayment)
    .then(() => {
      // ✅ Show success message
      Swal.fire({
        title: "Success!",
