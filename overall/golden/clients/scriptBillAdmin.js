...
function loadSavedPayments2() {
  ...
  firebase.database().ref("goldenwifi/monthly-bills").once("value").then(snapshot => {
    ...
    // 2. Add column headers for months
    ...
    // 3. Add buttons only for month columns (no changes to existing cells)
    rows.forEach(row => {
      const cells = row.querySelectorAll("td");
...
button.addEventListener("click", () => {
  ...
  if (currentStatus === "Unpaid") {
    ...
    // Update Firebase and reload after
    firebase.database().ref(path).update({ status: newStatus, actionTo: actionToStatus })
     ...
     const redo = confirm("Are you Sure You dont recieved Payment From this Client for this Month?");
    if (!redo) return;
    newStatus = "Unpaid";
    actionToStatus = "pending";

    // Update Firebase only (no confirmation or reload needed)
    firebase.database().ref(path).update({ status: newStatus, actionTo: actionToStatus });
  }

  // Immediate visual feedback (optional if reload is instant)
...
rows.forEach(row => {
  ...
  // Get number of existing columns in this row
  const currentCellCount = row.children.length;

  // If this row already has enough columns, skip appending
  if (currentCellCount >= theadRow.children.length) return;

  // Start appending missing month columns
  ...
  sortedMonthKeys.forEach(monthKey => {
    ...
    // Skip if the month label already exists in this row
    ...
    const td = document.createElement("td");
    td.setAttribute("data-month", monthKey); // For future detection
...
//openBtn.addEventListener("click", () => {
  modal.style.display = "block";
  loadUnpaidBillsWithFilters(); // load data every time modal opens
//}); 
