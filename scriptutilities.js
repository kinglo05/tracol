...
/////////////////// ADDING NEW MERCHANT TO DATABASE ////////////////////////
/////////////////// ADDING NEW MERCHANT TO DATABASE ////////////////////////
const submitNewMerchantButtonUser = document.getElementById('submit-merchant');

const roleInput= document.getElementById('roleinput');

submitNewMerchantButtonUser.addEventListener('click', () => {
  const newMerchantName = newMerchantNameInput.value; // Store the name for easier use
  const newMerchantAccount = {
      email: newMerchantName,
     emailLower: newMerchantName.toLowerCase(),
      password: newMerchantEmailInput.value,
      role : roleInput.value,
      
  };

  // 1. Check if the merchant name already exists:
  const merchantsRef = firebase.database().ref('user');
  merchantsRef.orderByChild('emailLower').equalTo(newMerchantAccount.emailLower).once('value', (snapshot) => {
      if (snapshot.exists()) {
          ...
          const updates2 = {};
          updates2['/users/' + newMerchantKey] = newMerchantAccount;
...
database.ref('mmogamit').on('value', (snapshot) => {
