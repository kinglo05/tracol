...
const totalTodayClaimed = document.getElementById("total-today");
...
/*  const assignTd = document.createElement("td");
  ...
  cb.checked = p.status === "claimed";
   cb.disabled = true;    //////use to freez the checkbox in claimed table
  ...
  [
    idxTd, amountTd, refTd, timeTd, merchantTd, dateTd, noteTd,
...
function recalcNumbersAndTotals() {
  ...
  Array.from(paymentsTBody.rows).forEach(tr => {
    const amt = parseFloat(tr.cells[1]?.textContent || "0");
    if (!Number.isNaN(amt)) total += amt;
   
  });
  const totalCell = document.getElementById("table-total");
  if (totalCell) totalCell.textContent = total.toFixed(2);
   if (totalTodayClaimed) totalTodayClaimed.value = total.toFixed(2);
    if (eightEl) eightEl.value = (total * 0.08).toFixed(2);
}
...
async function computeTodayNewTotal() {
  try {
    ...
    snap.forEach((child) => {
      ...
      if (p?.date === todayISO) {
        countToday += 1;
        sumToday1 += parseFloat(p.amount || 0);
      
      }


    });
   // if (totalTodayClaimed) totalTodayClaimed.value = sumToday1.toFixed(2);
    if (totalResiboEl) totalResiboEl.value = String(count);
   // if (eightEl) eightEl.value = (sumToday1 * 0.08).toFixed(2);
  } catch (e) {
  ...
  btnSaveEdit.addEventListener("click", async () => {
    try {
      ...
      const payload = {
        ...
        status: (editStatusInput.value || "claimed")
      };
